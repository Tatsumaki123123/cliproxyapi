...
// warnPrefix is prepended to the Status field to signal a successful-but-warned completion.
// GetAuthStatus recognises this prefix and returns status:"ok" with a "warning" field.
const warnPrefix = "\x00warn\x00:"

func RegisterOAuthSession(state, provider string) { oauthSessions.Register(state, provider) }

func SetOAuthSessionError(state, message string) { oauthSessions.SetError(state, message) }

// SetOAuthSessionWarning marks the session as successfully completed but with a non-fatal
// warning that the poller should surface to the user.
func SetOAuthSessionWarning(state, warning string) {
	oauthSessions.SetError(state, warnPrefix+strings.TrimSpace(warning))
}
