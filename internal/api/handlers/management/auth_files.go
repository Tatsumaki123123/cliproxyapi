...
func (h *Handler) RequestKiroToken(c *gin.Context) {
	...
	state := fmt.Sprintf("kro-%d", time.Now().UnixNano())
	authSvc := kiroauth.NewKiroAuth(h.cfg)

	// If a POST body with token_json is provided, use it directly.
	// This supports remote deployments where the Kiro IDE is on the user's local machine.
	var token *kiroauth.KiroCLIToken
	...
	if c.Request.Method == http.MethodPost {
		...
		if err := c.ShouldBindJSON(&body); err == nil && strings.TrimSpace(body.TokenJSON) != "" {
			parsed, errParse := kiroauth.ParseKiroCLIToken([]byte(body.TokenJSON))
			if errParse != nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("invalid token JSON: %v", errParse)})
				return
			}
			token = parsed
			tokenSource = "pasted token JSON"
		}
	}

	if token == nil {
		// Fall back to reading from the local Kiro CLI token file.
		localToken, tokenPath, err := authSvc.ImportFromKiroCLI()
		if err != nil {
			log.Errorf("Failed to import Kiro CLI token: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": fmt.Sprintf("Kiro token not found. Paste your token JSON from ~/.aws/sso/cache/kiro-auth-token.json or run 'kiro login' on this machine. (%v)", err),
			})
			return
		}
		token = localToken
	...
	log.Infof("Kiro token importing from: %s", tokenSource)
	fmt.Printf("Initializing Kiro authentication (source: %s)...\n", tokenSource)
	...
	go func() {
		...
		// Note: the CodeWhisperer API always uses us-east-1 regardless; sso_region
		// only affects which OIDC endpoint is used during token refresh.
		if ssoRegion := strings.TrimSpace(token.Region); ssoRegion != "" {
			metadata["sso_region"] = ssoRegion
			ts.Region = ssoRegion
		}

		// Resolve OIDC client credentials so that token refresh works without Kiro installed
		// on the server. Priority: fields embedded in the pasted JSON > local SSO cache lookup.
		resolvedClientID := strings.TrimSpace(token.ClientID)
		resolvedClientSecret := strings.TrimSpace(token.ClientSecret)
		if (resolvedClientID == "" || resolvedClientSecret == "") && token.ClientIDHash != "" {
		...
		if resolvedClientID != "" {
			...
			ts.ClientID = resolvedClientID
		}
		if resolvedClientSecret != "" {
			metadata["client_secret"] = resolvedClientSecret
			ts.ClientSecret = resolvedClientSecret
		}
		// auth_type: "aws_sso_oidc" when OIDC client credentials are available;
		// "kiro_desktop" for social-login tokens (no client_id/secret).
		if resolvedClientID != "" {
		...
		if resolvedClientID == "" {
			// Token was imported but no OIDC client credentials could be resolved.
			// The token will stop working after expiry because auto-refresh requires
			// client_id + client_secret. Signal the frontend to show a warning.
			SetOAuthSessionWarning(state, "kiro_no_oidc_creds")
			return
		}
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "url": "https://kiro.dev", "state": state})
}
...
func (h *Handler) GetAuthStatus(c *gin.Context) {
	...
	if status != "" {
		// A warnPrefix-prefixed status means the token was saved successfully but
		// with a non-fatal warning (e.g. no OIDC credentials for auto-refresh).
		if after, found := strings.CutPrefix(status, warnPrefix); found {
			CompleteOAuthSession(state)
			c.JSON(http.StatusOK, gin.H{"status": "ok", "warning": after})
			return
		}
