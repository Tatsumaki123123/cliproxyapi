...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	// On a 401/403 response, immediately trigger a background token refresh rather
	// than waiting for the next periodic expiry check. This handles tokens that
	// are revoked or expire before their recorded expires_at (e.g. Kiro /
	// CodeWhisperer returns 403 for an invalid bearer token, not 401).
	if !result.Success {
		if sc := statusCodeFromResult(result.Error); sc == http.StatusUnauthorized || sc == http.StatusForbidden {
			m.TriggerRefreshAsync(ctx, result.AuthID)
		}
	}
...
// TriggerRefreshAsync immediately schedules a background token refresh for the
// given auth, bypassing the normal expiry-based shouldRefresh check. Calling
// this after a 401/403 upstream response ensures the token is refreshed without
// waiting for the next periodic refresh cycle. No-op for API-key auths.
//
// The refresh always runs under context.Background() so it is not canceled
// when the triggering HTTP request context ends.
func (m *Manager) TriggerRefreshAsync(_ context.Context, authID string) {
	if authID == "" {
		return
	}
	// Skip API-key auths — they are static and have no refresh logic.
	m.mu.Lock()
	auth, ok := m.auths[authID]
	if !ok || auth == nil || auth.Disabled {
		m.mu.Unlock()
	...
	// Clear any pending-backoff so markRefreshPending accepts the request.
	...
	// Use background context: the refresh must outlive the triggering request.
...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	...
	// After a successful token rotation, clear non-quota model-level cooldowns
	// (e.g., 401 "unauthorized" backoffs). Quota cooldowns are intentionally
	// preserved — only authorization failures become stale after a token refresh.
	m.mu.Lock()
	if current := m.auths[id]; current != nil {
		stateChanged := false
		for _, state := range current.ModelStates {
			if state == nil || !state.Unavailable || state.Status == StatusDisabled {
				continue
			}
			if state.Quota.Exceeded {
				continue // quota cooldown is not affected by a token refresh
			}
			state.Unavailable = false
			state.NextRetryAfter = time.Time{}
			state.Status = StatusActive
