...
// autoImportKiroCLISQLite checks whether kiro-cli is installed on this machine
// (by looking for its SQLite database) and auto-registers a Kiro auth if:
//  1. The database file exists and contains a valid token, and
//  2. There is no existing Kiro auth already registered.
//
// This allows zero-config operation when kiro-cli is present on the server.
func (s *Service) autoImportKiroCLISQLite(ctx context.Context) {
	if s.coreManager == nil {
		return
	}
	// Skip if there is already at least one active kiro auth.
	for _, a := range s.coreManager.List() {
		if a != nil && strings.EqualFold(strings.TrimSpace(a.Provider), "kiro") && !a.Disabled {
			return
		}
	}

	authSvc := kiroauth.NewKiroAuth(s.cfg)
	token, dbPath, err := authSvc.ImportFromKiroCLISQLite("")
	if err != nil || token == nil {
		// Database not present or no valid token — silently skip.
		return
	}
	...
	if token.ClientID != "" {
		...
		ts.ClientID = token.ClientID
	} else {
		// Client credentials were not stored in SQLite (common with kiro-cli OIDC login).
		// Scan ~/.aws/sso/cache/ for any non-expired client registration so that OIDC
		// refresh works without requiring the Kiro IDE to be running on this machine.
		if regs, regErr := kiroauth.FindAllClientRegistrations(); regErr == nil && len(regs) > 0 {
			reg := regs[0]
	...
	if token.ClientSecret != "" {
		...
		ts.ClientSecret = token.ClientSecret
	}
	...
	auth := &coreauth.Auth{
		...
		Provider: "kiro",
		FileName: fileName,
		Label:    "Kiro (auto-imported from kiro-cli)",
		Storage:  ts,
		Metadata: metadata,
	}

	s.applyCoreAuthAddOrUpdate(ctx, auth)
	log.Infof("kiro: auto-imported token from kiro-cli SQLite database (%s) as auth %s", dbPath, fileName)
}
