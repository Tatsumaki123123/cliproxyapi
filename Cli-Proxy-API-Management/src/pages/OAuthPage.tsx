...
export function OAuthPage() {
  ...
  const kiroTokenFileRef = useRef<HTMLInputElement | null>(null);
  const [kiroFiles, setKiroFiles] = useState<KiroFileState>({
    tokenFileName: '',
    regFileName: '',
    tokenRaw: '',
    regRaw: '',
  ...
  // Auto-merge kiro files whenever either file content changes.
  useEffect(() => {
    const { tokenRaw, regRaw } = kiroFiles;
    if (!tokenRaw) return;
    try {
      const tokenObj = JSON.parse(tokenRaw) as Record<string, unknown>;
      const merged: Record<string, unknown> = { ...tokenObj };
      if (regRaw.trim()) {
        const regObj = JSON.parse(regRaw) as Record<string, unknown>;
        if (regObj.clientId) merged['clientId'] = regObj.clientId;
        if (regObj.clientSecret) merged['clientSecret'] = regObj.clientSecret;
      }
      setKiroFiles((prev) => ({ ...prev, mergeError: undefined }));
      updateProviderState('kiro', { tokenJson: JSON.stringify(merged, null, 2) });
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      setKiroFiles((prev) => ({ ...prev, mergeError: msg }));
    }
  }, [kiroFiles.tokenRaw, kiroFiles.regRaw]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleKiroTokenFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(event.target.files ?? []);
    if (!files.length) return;
    // Identify each file: token file contains accessToken, reg file contains clientId/clientSecret.
    ...
    void Promise.all(files.map(readFile)).then((contents) => {
      let tokenRaw = '';
      let tokenFileName = '';
      let regRaw = '';
      let regFileName = '';
      for (let i = 0; i < files.length; i++) {
        try {
          const obj = JSON.parse(contents[i]) as Record<string, unknown>;
          if (obj.accessToken) {
            tokenRaw = contents[i];
            tokenFileName = files[i].name;
          } else if (obj.clientId || obj.clientSecret) {
            regRaw = contents[i];
            regFileName = files[i].name;
          } else if (!tokenRaw) {
            // Fallback: treat first unrecognised file as token
            tokenRaw = contents[i];
            tokenFileName = files[i].name;
          }
        } catch {
      ...
      setKiroFiles((prev) => ({
        ...prev,
        ...(tokenRaw ? { tokenRaw, tokenFileName } : {}),
        ...(regRaw ? { regRaw, regFileName } : {}),
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        if (res.status === 'ok') {
          ...
          if (res.warning) {
            showNotification(t(`auth_login.kiro_oauth_warning_${res.warning}`), 'warning');
          }
  ...
  const startAuth = async (provider: OAuthProvider) => {
    ...
    try {
      const kiroTokenJson =
        provider === 'kiro' ? (states[provider]?.tokenJson || '').trim() : undefined;
      const res = await oauthApi.startAuth(
        provider,
        provider === 'gemini-cli'
          ? { projectId: projectId || undefined }
          : provider === 'kiro' && kiroTokenJson
            ? { tokenJson: kiroTokenJson }
            : undefined
      );
      updateProviderState(provider, {
        url: res.url,
            ...
            <div key={provider.id}>
              ...
              >
                <div className={styles.cardContent}>
                  ...
                  {provider.id === 'kiro' && (
                    <div className={styles.kiroTokenField}>
                      {/* Single multi-file picker: accepts token file + optional registration file */}
                      <div className={styles.formItem}>
                        <label className={styles.formItemLabel}>
                          {t('auth_login.kiro_files_label')}
                        </label>
                        <div className={styles.cardHintSecondary}>
                          {t('auth_login.kiro_files_hint')}
                        </div>
                        <div className={styles.filePicker}>
                          <Button
                            variant="secondary"
                            size="sm"
                            onClick={() => kiroTokenFileRef.current?.click()}
                            disabled={Boolean(state.polling)}
                          >
                            {t('auth_login.kiro_files_choose')}
                          </Button>
                          <div className={styles.kiroFileNames}>
                            {kiroFiles.tokenFileName && (
                              <span className={styles.kiroFileTag}>
                                📄 {kiroFiles.tokenFileName}
                              </span>
                            )}
                            {kiroFiles.regFileName && (
                              <span className={styles.kiroFileTag}>🔑 {kiroFiles.regFileName}</span>
                            )}
                            {!kiroFiles.tokenFileName && !kiroFiles.regFileName && (
                              <span className={styles.fileNamePlaceholder}>
                                {t('auth_login.kiro_files_placeholder')}
                              </span>
                            )}
                          </div>
                          <input
                            ref={kiroTokenFileRef}
                            type="file"
                            accept=".json,application/json"
                            multiple
                            style={{ display: 'none' }}
                            onChange={handleKiroTokenFileChange}
                          />
                        </div>
                      </div>
                      {kiroFiles.mergeError && (
                        <div className="status-badge error">
                          {t('auth_login.kiro_merge_error')}
                          {kiroFiles.mergeError}
                        </div>
                      )}
                      {/* Textarea: auto-populated by file merge, or manually pasted */}
                      <div className={styles.formItem}>
                        <label className={styles.formItemLabel}>
                          {t('auth_login.kiro_oauth_token_label')}
                        </label>
                        <div className={styles.cardHint}>
                          {t('auth_login.kiro_oauth_token_hint')}
                        </div>
                        <textarea
                          className={styles.kiroTokenTextarea}
                          value={state.tokenJson || ''}
                          disabled={Boolean(state.polling)}
                          onChange={(e) =>
                            updateProviderState(provider.id, { tokenJson: e.target.value })
                          }
                          placeholder={t('auth_login.kiro_oauth_token_placeholder')}
                          rows={5}
                        />
                      </div>
                    </div>
